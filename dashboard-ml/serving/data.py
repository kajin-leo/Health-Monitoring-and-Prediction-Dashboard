...
class MyPAData(BaseData):
    """
    require：all_df, feature_df, feature_names, all_IDs, max_seq_len, labels_df
    """
    def __init__(self, root_dir, file_list=None, pattern=None, n_proc=1, limit_size=None, config=None):
        self.set_num_processes(n_proc)
        self.config = config or {}
        ts_dir = os.path.join(root_dir, "PA_synthetic")
        label_path = os.path.join(root_dir, "individual_attributes.csv")

        # 1) read and concat time series
        self.all_df = self.load_all(ts_dir, file_list=file_list, pattern=pattern)
        self.all_df = self.all_df.sort_values(["ID", "t"]).set_index("ID")
        self.all_IDs = self.all_df.index.unique()

        # 2) set feature col
        # self.feature_names = ["f1", "f2", "f3", "f4", "f5", "f6"]
        self.feature_names = ["f1", "f2", "f3"] # without times
        self.feature_df = self.all_df[self.feature_names]

        # for pad/tranc
        self.max_seq_len = int(self.all_df.groupby(level=0).size().max())

        # 3) read label：BMI in individual_attributes.csv
        labs = pd.read_csv(label_path)
        id_col = "id" if "id" in labs.columns else labs.columns[0]
        if config["task"] == "regression":
            if "BMI" not in labs.columns:
                raise ValueError("No BMI in individual_attributes.csv")
            labs = labs[[id_col, "BMI"]].rename(columns={id_col: "ID", "BMI": "label"})
            labs["ID"] = labs["ID"].astype(str)
            labs = labs.set_index("ID").astype({"label": np.float32})
            self.labels_df = labs.loc[self.all_IDs]
        elif config["task"] == "classification":
            labs = pd.read_csv(label_path)[["participant_id", "HFZBMI", "AGE", "sex (1 male 2 female)"]]
            labs = labs.rename(columns={"participant_id": "ID"})
            labs["ID"] = labs["ID"].astype(str)

            cats = pd.unique(labs["HFZBMI"].astype(str).str.strip())
            labs["label"] = pd.Categorical(labs["HFZBMI"].astype(str).str.strip(), categories=cats).codes.astype(np.int64)
            self.class_names = list(cats)
            self.num_classes = len(self.class_names)

            self.labels_df = labs.set_index("ID").loc[self.all_IDs, ["label"]]

            demo = labs[["ID", "AGE", "sex (1 male 2 female)"]].set_index("ID")
            s_age_months = demo["AGE"].astype(np.float32)
            s_is_female  = (demo["sex (1 male 2 female)"] == 2).astype(np.float32)   # 2=Female → 1.0；1=Male → 0.0

            # self.feature_df["age"] = self.feature_df.index.map(s_age_months.to_dict())
            # self.feature_df["is_female"]  = self.feature_df.index.map(s_is_female.to_dict())
            # self.feature_names += ["age", "is_female"]
            self.static_feat_names = ["age", "is_female"]
            self.static_df = pd.DataFrame(
                {"age": s_age_months, "is_female": s_is_female},
                index=demo.index
            ).astype(np.float32).loc[self.all_IDs]

        if limit_size is not None:
            n = int(limit_size * len(self.all_IDs)) if 0 < limit_size <= 1 else int(limit_size)
            keep = self.all_IDs[:n]
            self.all_IDs = keep
            self.feature_df = self.feature_df.loc[keep]
            self.labels_df = self.labels_df.loc[keep]

    def load_all(self, ts_dir, file_list=None, pattern=None):
        if file_list is None:
            paths = sorted(glob.glob(os.path.join(ts_dir, "*")))
        else:
            paths = [os.path.join(ts_dir, p) for p in file_list]
        # if pattern:
        #     paths = [p for p in paths if pattern in os.path.basename(p)]
        if not paths:
            raise FileNotFoundError(f"No csv under {ts_dir}")

        if self.n_proc > 1:
            from multiprocessing import Pool
            with Pool(processes=min(self.n_proc, len(paths))) as pool:
                parts = pool.map(self.load_single, paths)
        else:
            parts = [self.load_single(p) for p in paths]
    ...
    def load_single(self, filepath):
        try:
            ...
            if df.shape[1] < 8:
                raise ValueError
            df = df.iloc[:, 2:8:2].copy()
        except Exception:
            df = pd.read_csv(filepath, header=None)
            if df.shape[1] < 8:
                raise ValueError(f"{filepath} 少于8列")
            df = df.iloc[:, 2:8:2].copy()
